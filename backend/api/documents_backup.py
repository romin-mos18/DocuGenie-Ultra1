...
def process_document_with_docling_enhanced_sync(file_path: str, file_type: str, doc_id: int):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Entity extraction failed, using empty result: {entity_error}")
        ...
        if doc_id in document_storage:
            ...
            entity_count = entities_result.get('entity_count', 0) if isinstance(entities_result, dict) else 0
            ...
            logger.debug(f"🔍 Classification details for {doc_id}:")
            logger.debug(f"   Text length: {len(extracted_text)} characters")
            logger.debug(f"   Classification success: {classification_result.get('success', False)}")
            logger.debug(f"   Raw classification: {classification_result}")
    ...
    try:
        ...
        for doc_id, doc_data in document_storage.items():
            ...
            documents.append({
                ...
                "documentType": formatted_doc_type,  # Frontend compatibility
...
async def upload_document(
    ...
    try:
        ...
        try:
            ...
            # Process immediately to get classification results
            process_document_with_docling_enhanced_sync(file_path, file_type, doc_id)
            logger.info(f"✅ Document processed and classified: {file.filename}")
            
            # Verify processing completed
            processed_doc = document_storage.get(doc_id, {})
            if processed_doc.get("status") != "processed":
                logger.warning(f"⚠️ Processing may not have completed properly for: {file.filename}")
                
        except Exception as e:
            logger.error(f"❌ AI processing failed for {file.filename}: {e}")
            # Try simple classification without Docling
            try:
                logger.info(f"🔄 Attempting fallback classification...")
                ...
                if text_content.strip():
                    ...
                    # Update storage with classification results
                    document_storage[doc_id].update({
                        ...
                        "ai_analysis": {
                            "classification": classification_result,
                            "processing_method": "fallback_classification"
                        }
                    })
                    logger.info(f"✅ Fallback classification completed: {classification_result.get('document_type', 'unknown')}")
                else:
        ...
        # Get the processed document data to include classification results
        processed_doc = document_storage.get(doc_id, {})
